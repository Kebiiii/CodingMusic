...
public class MainActivity extends BaseActivity {
	...
	public void change(int position) {
		...
		if (pager.getCurrentItem() == 0) {
			myMusicListFragment.loadData();//在这里加载数据保证在绑定服务之后执行
			myMusicListFragment.changeUIStatusOnPlay(position);
		} else if (pager.getCurrentItem() == 1) {
			//netMusicListFragment.changeUIStatus(position);
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			case R.id.favorite:
				...
				break;
			case R.id.latest_music:
				Intent intent1 = new Intent(this,PlayRecordListActivity.class);
				startActivity(intent1);
				break;
			case R.id.exit:
				stopService(new Intent(this, PlayService.class));//停止服务
				finish();//结束活动，会unbindService
				break;
